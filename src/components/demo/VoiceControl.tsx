/**
 * xx. Secrets Demo's fork of the voice control (copied from
 * src/components/voice/VoiceControl.tsx) — iterates on the composer
 * interaction without touching the shared control:
 *
 *   Hold: the orb stretches into a pill — pure width growth, the capsule
 *   radius never changes — and the state's name ("Listening…") rides
 *   inside the pill beside the dots instead of above the dock.
 *   Tap the pill at any point: it blooms into a full feedback sheet
 *   ("Connecting…" + the processing note) that melts away when the
 *   session lands back at idle.
 */
...
import { EdgeAura } from '../voice/EdgeAura'
import { LiquidDistortion } from '../voice/LiquidDistortion'
import { useVoiceInput, type VoiceStatus } from '../voice/useVoiceInput'
import { VoiceGlyph } from '../voice/VoiceGlyph'
...
/** Hardcoded ephemeral replies per context — the demo speaks in scripted
    turns (nothing is actually heard yet). Each context alternates a short
    line and a long one, so repeated holds show both registers. `{place}`
    is filled with the focused place's name at pick time. */
const EPHEMERAL_REPLIES: Record<'home' | 'browse' | 'details', string[]> = {
  // The home / projects board — status at a glance.
  home: [
    'You\u2019re covered for the weekend \u2014 three reservations, nothing needs you yet.',
    'This weekend\u2019s set: dinner at Valette tonight, the Healdsburg stay checks in Friday, and your flight\u2019s on time so far. Kyoto planning still has two tasks open when you\u2019re ready.',
  ],
  // Browsing the results in the list / map view.
  browse: [
    'Valette\u2019s still the front-runner \u2014 88% match, and a 6-minute walk from where you\u2019re staying.',
    'I compared all four on what you asked for \u2014 date-night feel, walkable, not stuffy. Valette leads at 88% with the strongest service notes; Chalkboard is the livelier pick if you\u2019d rather keep it casual. Both have tables Saturday.',
  ],
  // A restaurant's details sheet is open.
  details: [
    '{place} has a 7:30 table for two on Saturday \u2014 want me to hold it?',
    'Reviews come back to three things: the duck confit, desserts worth saving room for, and service that treats a birthday right. It gets loud after 8, so if you want conversation, aim for the earlier seating.',
  ],
}

/** The reply's word stream — slow enough to visibly build (the card's
    bloom takes ~0.3s, so the stream starts after it), plus the beat the
    card holds after the last word so the line can actually be read. */
const STREAM_START_MS = 400
const STREAM_WORD_MS = 90
const READ_HOLD_MS = 1400
/** useVoiceInput's responding window — the card outlives it when a long
    reply is still streaming. */
const RESPOND_WINDOW_MS = 2600

/** Orb diameter — thumb scale, comfortably tappable (≥ 44pt hit target). */
const ORB_SIZE = 72

/** The live-session pill: the orb stretched wide while listening /
    thinking / responding, dots keeping their seat with the state's name
    beside them. Height stays ORB_SIZE so the capsule radius never moves —
    the circle just grows sideways. */
const VOICE_PILL_W = 208
...
export function VoiceControl({
  ...
  const { status, transcript, error, levelRef, start, finish } = useVoiceInput()
  ...
  // The live-session pill: the press itself starts the stretch (getUserMedia
  // can lag; the growth shouldn't wait for it) and the pill holds through
  // the whole loop. The follow-up pill, inline dock and trip file each own
  // the dock's shape already — they win.
  const voicePill = !isPill && !inline && !tripOpen && (pressing || status !== 'idle')
  // While the press hasn't resolved to a status yet, the pill already reads
  // as the session it's becoming.
  const voicePillLabel = status === 'idle' ? STATE_META.listening.label : meta.label

  // The feedback sheet — tap the live pill and it blooms into a full dark
  // surface carrying the processing note. It melts away on its own when
  // the loop lands back at idle; a tap puts it away sooner.
  const [sheetOpen, setSheetOpen] = useState(false)
  // Ephemeral response delivery — the agent's reply rides the card
  // everywhere the dock lives (home, projects, list results, details):
  // when the loop reaches responding and no transaction flow is consuming
  // the moment, the pill blooms into the card on its own carrying the
  // context's next scripted line.
  const replyCursor = useRef<Record<string, number>>({})
  const [cardReply, setCardReply] = useState<string | null>(null)
  // The card doesn't slam shut when the loop lands at idle — a long reply
  // may still be streaming, so it stays up until the last word has landed
  // plus a reading beat.
  useEffect(() => {
    if (status !== 'idle' || !sheetOpen) return
    const words = cardReply ? cardReply.split(' ').length : 0
    const streamMs = STREAM_START_MS + words * STREAM_WORD_MS
    const holdMs = Math.max(0, streamMs - RESPOND_WINDOW_MS) + READ_HOLD_MS
    const timer = window.setTimeout(() => setSheetOpen(false), holdMs)
    return () => clearTimeout(timer)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [status, sheetOpen])

  useEffect(() => {
    if (status !== 'responding' || stage !== 'none' || tripOpen) return
    const ctx = flow?.focusedPlace ? 'details' : flow ? 'browse' : 'home'
    const lines = EPHEMERAL_REPLIES[ctx]
    const i = replyCursor.current[ctx] ?? 0
    replyCursor.current[ctx] = (i + 1) % lines.length
    setCardReply(lines[i].replace('{place}', flow?.focusedPlace ?? ''))
    setSheetOpen(true)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [status, stage, tripOpen])
  ...
  const onPointerDown = () => {
    ...
    if (status === 'idle') {
      ...
      // thinking / responding: the pill is live feedback — tapping it
      // opens the full sheet instead of being ignored.
      setSheetOpen(true)
    }
  }
  ...
  const simulateUtterance = () => {
    if (!flow) return
    if (flow.stage === 'none') {
      // At rest a hold yields the context's scripted ephemeral reply (the
      // card) — even with a place focused. The booking flow starts from
      // the sheet's "Get reservation" chip, not from the hold.
      return
  ...
  const onPointerUp = () => {
    ...
    if (press.startedListening) {
      ...
      finish() // tapped while listening → send, and open the sheet on it
      simulateUtterance()
      setSheetOpen(true)
    }
  }
  ...
  const restingHint =
    ...
    !error
      ...
      : // The live pill carries its own name inside — the slot above goes
        // quiet (NBSP holds its height) instead of doubling the label.
        (error ?? (voicePill ? '\u00A0' : meta.label))
  const label =
    stage === 'followUp'
      ? status === 'listening'
        ? 'Listening\u2026'
  ...
  return (
    ...
    <div className="relative flex min-h-0 flex-1 flex-col px-4 pt-[var(--safe-top)] pb-[calc(var(--safe-bottom)+24px)]">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col">
        ...
        {/* The card is the reply's surface — only the live transcript
            floats here now (the hook's mic-fallback copy never shows;
            the demo speaks in scripted turns). */}
      ...
      >
        ...
        <AnimatePresence>
          {dockAux && !isPill && !inline && !tripOpen && !voicePill && (
            ...
            width: isPill
              ...
              : inline
                ...
                : voicePill
                  ? VOICE_PILL_W
                  : ORB_SIZE,
            // Height never moves for the live pill — the capsule radius
            // stays put and the circle purely grows sideways.
        ...
        >
          ...
          {isPill && flow ? (
            ...
            <span className="pointer-events-none relative flex items-center gap-2">
              ...
              {/* The state's name, riding inside the pill beside the dots —
                  keyed on the label so listening → thinking crossfades;
                  popLayout keeps the swap from jolting the centered row. */}
              <AnimatePresence initial={false} mode="popLayout">
                {voicePill && (
                  <motion.span
                    ...
                    className="text-[14px] font-medium whitespace-nowrap text-white/85"
                  >
                    {voicePillLabel}
                  </motion.span>
                )}
      ...
      {/* The feedback card — the live pill grown into a dark card holding
          the bottom third of the frame: dots at card scale, the connection
          headline, and the processing note (the reference's "agent is
          working" moment). Same material, same constant capsule-family
          radius — it reads as the pill having grown, not a new surface.
          It melts away when the loop lands at idle; a tap puts it away
          sooner. */}
      <AnimatePresence>
        {sheetOpen && (
          <motion.button
            ...
            className="absolute inset-x-4 bottom-4 z-50 flex h-[29%] flex-col items-center justify-center overflow-hidden rounded-[36px] bg-[#131117] px-8 text-center outline-none"
          ...
          >
            {/* The dots step back once the reply starts arriving — scaled
                down and their row squeezed so the words get the room. The
                reply state lingers past `responding` (the stream outlives
                the loop), so it keys on the card's own reply, not status. */}
            <motion.div
              className="flex items-center justify-center"
              initial={false}
              animate={
                cardReply && (status === 'responding' || status === 'idle')
                  ? { scale: 0.5, height: 34 }
                  : { scale: 1, height: 56 }
              }
              transition={{ type: 'spring', stiffness: 300, damping: 26 }}
            >
              <VoiceGlyph status={status} levelRef={levelRef} size={56} />
            </motion.div>
            {/* No initial={false} here: the card often mounts straight into
                the reply state (auto-bloom at responding), and suppressing
                initials would paint every word at once instead of letting
                the stream build. */}
            <AnimatePresence mode="wait">
              {cardReply && (status === 'responding' || status === 'idle') ? (
                // The reply itself is the content — no "Responding" headline.
                // It builds out word by word like the agent speaking it, and
                // reads left-aligned like a message, not a title card.
                <motion.span
                  key="reply"
                  className="mt-4 w-full text-left text-[16px] leading-[1.5] tracking-[-0.01em] text-white/90"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0, filter: 'blur(6px)' }}
                  transition={{ duration: 0.2 }}
                >
                  {cardReply.split(' ').map((word, i) => (
                    // Opacity-only per word — inline spans keep natural text
                    // wrapping (transforms/filters don't apply to inlines).
                    <motion.span
                      key={`${word}-${i}`}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{
                        delay: STREAM_START_MS / 1000 + i * (STREAM_WORD_MS / 1000),
                        duration: 0.3,
                      }}
                    >
                      {word}{' '}
                ...
                >
                  <span className="mt-5 text-[16px] font-medium tracking-[-0.01em] text-white">
                    Connecting&hellip;.
                  </span>
                  <span className="mt-2 max-w-[300px] text-[12.5px] leading-relaxed text-white/60">
                    Agent is processing your request. Please wait, it might take a few
                    seconds.
                  </span>
                </motion.span>
              )}
