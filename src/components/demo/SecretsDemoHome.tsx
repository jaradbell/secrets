/**
 * xx. Secrets Demo — a 1:1 fork of 5A's Moodboard (components copied to
 * src/components/demo/) as a sandbox to build the demo on without touching
 * the 5A prototype.
 *
...
import { MatchStyleProvider } from '../transaction/MatchRing'
...
/** Files rows that are doorways — tap to drop into the conversation
    (the 1C altitude move). Rows not listed here stay inert sketch. */
...
export function SecretsDemoHome() {
  ...
  // Altitude — the home (both faces) and the conversation it drops into.
  // Tapping a doorway row in the Files state passes through the goo: the
  // veil rises, the thread mounts underneath, the veil melts (1C's move).
  ...
  // The liquid pause between altitudes — rendered on both sides of the
  // swap so the veil covers the mount and the melt alike.
  const gooPortal =
    viewport &&
    createPortal(
      pending && (
  ...
  if (level === 'thread') {
    return (
      <>
        {/* 7J's match language on the decide surfaces — the combo capsule
            (rank dot + "% match") tucked below each row's content, white
            pill POIs on the map. */}
    ...
    <VoiceControl
      ...
      idleContent={
        <div className="relative h-full w-full">
          {view === 'assistant' ? (
            ...
            >
              {/* All three states — Upcoming, Files (the priority projects and
                  loose threads), Connect — cycled by the notch rail. Rows in
                  THREAD_TITLES are doorways into their conversation. */}
              <HomeStates
                pages={['upcoming', 'files', 'connect']}
                onOpenProject={(id) => {
                  const title = THREAD_TITLES[id]
                  if (title) enterThread(title)
                }}
              />
            </motion.div>
          ) : (
          ...
          {/* The goo veil covering the drop into a conversation. */}
          {gooPortal}
        </div>
      }
    />
